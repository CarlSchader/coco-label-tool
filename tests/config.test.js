...
describe('CONFIG.canvas properties', () => {
  ...
  test('hover radius is larger than point radius', () => {
    expect(CONFIG.canvas.pointHoverRadius).toBeGreaterThan(CONFIG.canvas.pointRadius);
  });

  test('hover glow radius is larger than hover radius', () => {
    expect(CONFIG.canvas.pointHoverGlowRadius).toBeGreaterThan(CONFIG.canvas.pointHoverRadius);
  });
});
...
describe('CONFIG.colors properties', () => {
  ...
  test('rgba color has valid format', () => {
    expect(CONFIG.colors.segmentation).toMatch(/^rgba\(\d+,\s*\d+,\s*\d+,\s*[\d.]+\)$/);
  });
});
